...
- 主 Agent 只加载 `research-protocol` Skill，用于约束全流程。
- `WorkspaceBootstrapper` 会复制全部 Skill 目录，便于工作区保持完整。
- `build_subagent_registry()` 明确丢弃 `skill_paths`；四个窄化子 Agent 使用各自的 system prompt 和结构化响应 schema，没有通用文件系统、todo 或 Skill 读取能力。
...
- `save_screening_decision(project_id, included_paper_ids, excluded_paper_ids, reasons)`：原子保存筛选决定并进入 `SCREENED`。
...
`verify_doi(doi)` 仍在文献工具模块中实现，但当前没有分配给主 Agent 或任何子 Agent。Reviewer 把 DOI 当作论文元数据，实际审查集中在 `claim`、`evidence_id`、原文引句、页码/章节和结论之间的对应关系。
...
- `api/app.py`：提供健康检查、普通调用和 SSE 流式调用。
...
- Scout：保存 `SearchReport`，`CREATED → SEARCHED`。
...
`ResearchWorkflowGuardMiddleware` 还会执行以下硬约束：项目必须先创建；只能委派四个注册角色；角色必须处于指定阶段；上一份待提交结果必须先提交；同一任务的 Scout 只能调用一次。这些检查位于中间件中，不依赖模型自行遵守提示词。
