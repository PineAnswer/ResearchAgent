...
项目采用 Supervisor–Worker 多 Agent 模式：`research-supervisor` 负责流程编排，七个窄化子 Agent 分别负责检索、单篇阅读、综合、证据审查、提纲设计、分节写作和总编整合。
...
- `domain/models.py`：定义 `ResearchProject`、`SearchReport`、`PaperCard`、`Evidence`、`SynthesisReport`、`ReviewResult`、`ReviewOutline`、`SectionDraft` 和 `NarrativeReview` 等 Pydantic 数据契约。
...
执行管线故障、模型超时或连续结构校验失败会保存 `RuntimeIssue` 并保持当前阶段。判断具体原因时必须读取 `InsufficientEvidence.reason`、`RuntimeIssue.reason` 和运行事件；`INCONCLUSIVE` 在当前状态机中不可重新打开。
...
Tool 是可调用函数，例如 `search_multi_source(query, limit)`、`search_library(query, limit)`、`fetch_paper_text(paper_id, doi, url, max_pages)`、`commit_subagent_result(subagent_type)`。参数由框架根据函数签名暴露给模型，函数返回 JSON 字符串或结构化结果。
...
|---|---|---|---|
| `literature-scout` | `search_library`、`search_multi_source` | `SearchReport` | 多条短查询覆盖 OpenAlex、Crossref、Semantic Scholar 和 arXiv；中间件捕获原始结果并重建候选集 |
...
| `chief-editor` | `get_active_research_project` | `NarrativeReview` | 仅在 `OUTLINED` 委派；整合全部草稿后直接进入 `COMPLETED` |
...
前端 / CLI / HTTP API
      ↓
ResearchSupervisor ──→ 主 Agent / 七个窄化子 Agent
      ↓                         ↓
...
- Chief editor：保存 `NarrativeReview`，`OUTLINED → COMPLETED`，流程随即结束。
...
`ResearchWorkflowGuardMiddleware` 还会执行以下硬约束：项目必须先创建或由 Supervisor 绑定恢复；只能委派七个注册角色；角色必须处于指定阶段；上一份待提交结果必须先提交；同一任务的 Scout 只能调用一次；`SEARCH_REVIEW_PENDING` 阶段的确认和停止只能来自用户反馈 API。这些检查位于中间件中，不依赖模型自行遵守提示词。
