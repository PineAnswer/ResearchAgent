...
搜索工具返回结构化错误时，严格执行其中的instruction；仅当错误明确要求停止或外部检索已经尝试后，才保留此前成功结果并输出 SearchReport。本地结果为空且尚未调用search_openalex时禁止输出SearchReport。工具调用次数只统计实际返回ToolMessage的调用；被限额中间件拦截的调用意图不得计为已执行。
